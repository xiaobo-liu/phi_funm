...
%       time   - Execution time breakdown for code profiling:
%                time(1): Parameter selesction, including norm estimations.
%                time(2): Evaluating the Pade approximant via Paterson-Stockmeyer.
%                time(3): Recovering phase.
%
% Reference:
%
...
runtime(1) = toc(para_time);

eval_time = tic;
A = A./2^s;                               
[N, D] = pade_coef(m,p);   
runtime(2) = toc(eval_time);
...
for i=1:s
    ...
    runtime(3) = runtime(3) + toc(recv_time);
end
